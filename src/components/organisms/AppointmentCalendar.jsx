...
const getPatientName = (patientId) => {
    ...
    return patient ? (patient.name_c || patient.Name || 'Unknown Patient') : "Unknown Patient"
  }

  const getDoctorName = (doctorId) => {
    const doctor = staff.find(s => s.Id === parseInt(doctorId))
    return doctor ? (doctor.name_c || doctor.Name || 'Unknown Doctor') : "Unknown Doctor"
  }
...
const getTodayAppointments = () => {
    ...
    return appointments.filter(apt => {
      ...
      const timeA = a.time_c || a.time || ''
      const timeB = b.time_c || b.time || ''
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
                        <span className="flex items-center">
                          ...
                          Dr. {getDoctorName(appointment.doctor_id_c || appointment.doctorId)}
                        </span>
                        <span className="flex items-center">
                          <ApperIcon name="Building2" className="h-4 w-4 mr-1" />
                          {appointment.department_c || appointment.department || ''}
                        </span>
                        <span className="flex items-center">
                          <ApperIcon name="Clock" className="h-4 w-4 mr-1" />
                          {formatTime(appointment.time_c || appointment.time || '')}
                        </span>
                      </div>
                    </div>
                    
                    <div className="flex items-center space-x-3">
                      <StatusIndicator status={appointment.status_c || appointment.status || ''} type="appointment" />
...
{todayAppointments.filter(a => (a.status_c || a.status || '') === "completed").length}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-bold text-blue-900">
                  {todayAppointments.filter(a => (a.status_c || a.status || '') === "scheduled").length}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-bold text-red-900">
                  {todayAppointments.filter(a => (a.status_c || a.status || '') === "cancelled").length}
                </p>
              </div>
            </div>
          </CardContent>
